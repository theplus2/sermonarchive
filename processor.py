...
def sync_files(target_folder, progress_callback=None, status_callback=None):
    ...
    # DB에 있지만 폴더에 없는 파일 삭제 (파일명 변경/삭제된 경우)
    c.execute("SELECT file_name FROM sermons")
    db_filenames = set(row[0] for row in c.fetchall())
    ...
    for i, file_path in enumerate(files):
        ...
        # 변경 확인
        c.execute("SELECT last_modified FROM sermons WHERE file_name=?", (filename,))
        row = c.fetchone()
        
        if row and row[0] == mtime:
            continue # 변경 없으면 스킵
            
        if status_callback: status_callback(f"처리 중: {filename}")
        
        content = ""
        if file_path.lower().endswith(".docx"): content = extract_text_from_docx(file_path)
        elif file_path.lower().endswith(".hwp"): content = extract_text_from_hwp(file_path)
